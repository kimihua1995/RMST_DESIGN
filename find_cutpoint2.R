rmst.reg <- function(dat, tau){
  ...
  #spline.x <- ns(dat$X, df = 3)
  #fit <- lm(dat$y ~ 1 + spline.x, weights = dat$weight)
  return(fit)
}
...
cut.pred1 <- function(dat0, dat1, tau, t.change, t.analysis){
  ...
  beta1 <- ifelse(abs(coef(fit1)[3]/sqrt(vcov(fit1)[3,3])) <= qnorm(0.975), 0, coef(fit1)[3])
...
cut.pred3 <- function(dat0, dat1, tau, t.analysis){
  ...
  if (k1 == 0){
    ...
    beta1 <- ifelse(abs(coef(fit1)[2]/sqrt(vcov(fit1)[2,2])) <= qnorm(0.975), 0, coef(fit1)[2])
    t.change1 <- c(tau,tau)
  }else if (k1 == 1){
    lambda1 <- exp(coef(fit1)[c(1,2,2)])
    beta1 <- ifelse(abs(coef(fit1)[3]/sqrt(vcov(fit1)[3,3])) <= qnorm(0.975), 0, coef(fit1)[3])
    ...
    beta1 <- ifelse(abs(coef(fit1)[4]/sqrt(vcov(fit1)[4,4])) <= qnorm(0.975), 0, coef(fit1)[4])
